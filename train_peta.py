...
def main(cfg, args):
    ...
    timestamp = time_str()
    stdout_file = os.path.join(log_dir, f'stdout_multi_evavit_swinT_peta.txt')
    save_model_path = os.path.join(model_dir, f'ckpt_max_multi_evavit_swinT_peta.pth')
    ...
    if cfg.DATASET.TYPE == 'pedes':
        train_set = PedesAttrPETA(cfg=cfg, split=cfg.DATASET.TRAIN_SPLIT, transform=train_tsfm,
