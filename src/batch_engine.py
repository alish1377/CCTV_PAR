...
def logits4pred(criterion, logits_list):
    if criterion.__class__.__name__.lower() in ['bceloss', 'scaledbceloss', 'gradnormloss', 'focalloss']:
...
def batch_trainer(cfg, args, epoch, model, model_ema, train_loader, criterion, optimizer, loss_w=[1, ],
                  pos_recall=None, neg_recall=None, scheduler=None, tb_writer=None, device=None):
    model.train()
    ...
    with tqdm(desc='Epoch %d - Training' % epoch, unit='it', total=len(train_loader)) as pbar:
        for step, (imgs, gt_label, imgname) in enumerate(train_loader):
            ...
            gt_label = gt_label.to(device) if device is not None else gt_label
            train_logits, _ = model(imgs, gt_label)

            if cfg.LOSS.TYPE == "gradnormloss":
                loss_list, loss_mtr = criterion(train_logits, gt_label, pos_recall, neg_recall)
                weight_optimizer = torch.optim.Adam([criterion.w.data], lr=0.0008)

                # print("weight criterion")
                # print(criterion.w)
                actual_model = model.module if hasattr(model, 'module') else model
                criterion.additional_forward_and_backward(actual_model.classifier.logits, weight_optimizer)
            if cfg.LOSS.TYPE == "scaledbceloss":
                loss_list, loss_mtr = criterion(train_logits, gt_label, pos_recall, neg_recall)
            else:
                loss_list, loss_mtr = criterion(train_logits, gt_label, pos_recall, neg_recall)
            ...
            # train_loss = train_loss / train_loss.detach()

            optimizer.zero_grad()
            # Compute the reciprocal of the loss values and normalize them
            # train_loss = train_loss / train_loss.detach()
            train_loss.backward()
...
def valid_trainer(cfg, args, epoch, model, valid_loader, criterion, pos_recall=None, neg_recall=None, loss_w=[1, ], device=None):
    model.eval()
    loss_meter = AverageMeter()
    subloss_meters = [AverageMeter() for i in range(len(loss_w))]
    ...
    with tqdm(desc='Epoch %d - Validation' % epoch, unit='it', total=len(valid_loader)) as pbar:
        with torch.no_grad():
            for step, (imgs, gt_label, imgname) in enumerate(valid_loader):
                ...
                if cfg.LOSS.TYPE == "gradnormloss":
                    loss_list, loss_mtr = criterion(valid_logits, gt_label, pos_recall, neg_recall)
                elif cfg.LOSS.TYPE == "scaledbceloss":
                    loss_list, loss_mtr = criterion(valid_logits, gt_label, pos_recall, neg_recall)
                else:
                    loss_list, loss_mtr = criterion(valid_logits, gt_label, pos_recall, neg_recall)
