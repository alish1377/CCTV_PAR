...
def tb_visualizer_pedes(tb_writer, lr, epoch, train_loss, valid_loss, train_result, valid_result,
                        train_gt, valid_gt, train_loss_mtr, valid_loss_mtr, model, attr_name=None):
    ...
    tb_writer.add_scalar('train_perf/pos_recall', np.mean(train_result.filtered_label_pos_recall), epoch)
    ...
    tb_writer.add_scalar('test_perf/pos_recall', np.mean(valid_result.filtered_label_pos_recall), epoch)
    tb_writer.add_scalars(
      'test_perf/vector_pos_recall',
      {f'class_{i}': val for i, val in enumerate(valid_result.label_pos_recall)},
      epoch
    )
    tb_writer.add_scalar('test_perf/neg_recall', np.mean(valid_result.label_neg_recall), epoch)
    tb_writer.add_scalars(
      'test_perf/vector_neg_recall',
      {f'class_{i}': val for i, val in enumerate(valid_result.label_neg_recall)},
      epoch
    )
