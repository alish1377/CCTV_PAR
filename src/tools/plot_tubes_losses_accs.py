...
def plot_resolution_brightness_vs_acc(image_resolutions, image_brightness, upper_accs, lower_accs, save_path):
    """
    Plots the effect of image resolution and brightness on model accs.

    Parameters:
    - image_resolutions: List of total pixels per image (width * height)
    - image_brightness: List of average brightness values (0_1 scale)
    - upper_accs: List of acc values for the upper branch
    - lower_accs: List of acc values for the lower branch
    """

    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    # ---- Plot 1: Acc vs. Resolution ----
    axes[0].plot(image_resolutions, upper_accs, 'o', linestyle='', label='Upper Body Acc')
    axes[0].plot(image_resolutions, lower_accs, '*', linestyle='', label='Lower Body Acc')
    axes[0].set_title("Acc vs. Resolution")
    axes[0].set_xlabel("Image Resolution (width * height)")
    axes[0].set_ylabel("Acc")
    axes[0].legend()
    axes[0].grid(True)

    # ---- Plot 2: Acc vs. Brightness ----
    axes[1].plot(image_brightness, upper_accs, 'o', label='Upper Body Acc')
    axes[1].plot(image_brightness, lower_accs, '*', label='Lower Body Acc')
    axes[1].set_title("Acc vs. Brightness (V)")
    axes[1].set_xlabel("Average Brightness (V channel)")
