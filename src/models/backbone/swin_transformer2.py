...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.norm_1 = nn.LayerNorm(embed_dims*8)
        self.norm_2 = nn.LayerNorm(embed_dims*8)
        ...
        self.cross_att_eva_swin = CrossAttention(dim=embed_dims*8)
        self.cross_att_swin_eva = CrossAttention(dim=embed_dims*8)
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is None:
            ...
            ckpt = None
            if validators.url(pretrained):
              ckpt = torch.hub.load_state_dict_from_url(pretrained, map_location=lambda storage, loc: storage)
            else:
            ...
            # print(_state_dict.keys())
            # print("_state_dict keys first element:")
            ...
            # strip prefix of state_dict(module.)
            ...
            #print("differences between state_dict keys")
            #print(self.state_dict().keys())
            #print(state_dict.keys())
    ...
    def forward(self, x, semantic_weight=None):
        ...
        for i, (stage, tab) in enumerate(zip(self.stages, self.tablocks)):
            x = rearrange(x, 'b (h w) c -> b c (h w)', h=hw_shape[0], w=hw_shape[-1])
            x = tab(x)
            x = rearrange(x, 'b c (h w) -> b (h w) c', h=hw_shape[0], w=hw_shape[-1])
...
# /content/drive/MyDrive/Pedestrain_Attribute_Recognition/C2T_Net/weights/best_model.pth
...
def swin_small_patch4_window7_224(pretrained="/content/drive/MyDrive/Pedestrain_Attribute_Recognition/Rethinking_of_PAR/weights/swin_small_patch4_window7_224.pth", convert_weights=True, img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
...
def swin_tiny_patch4_window7_224(pretrained=None, convert_weights=False, img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
