...
class LinearClassifier(BaseClassifier):
    def __init__(self, nattr, c_in, bn=False, adapt=False, pool='avg', scale=1):
        super().__init__()
        self.adapt = adapt
        ...
        self.adaptformer = nn.Sequential(
            nn.Linear(c_in, 8),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(8, nattr)
    ...
    def forward(self, feature, label=None):
        cosine = None
        if self.adapt:
            # Apply Layer Normalization to the feature
            residual = feature
            x_tmp = self.layer_norm(feature)
            y_mlp = self.logits(x_tmp)
            y_adaptformer = self.adapt_scale * self.adaptformer(x_tmp)
            x = y_mlp + y_adaptformer

        else:
            #feature = self.pool(feature).view(feature.size(0), -1)
            x = self.logits(feature)

            classifier_n = F.normalize(self.logits[1].weight, dim=1)  # Change index form 0 to 1
...
class FeatClassifier(nn.Module):
    ...
    def __init__(self, backbone, classifier, bn_wd=True, use_sigmoid=False):
